...
// src/modules/cars/cars.routes.test.ts
...
describe("Cars routes", () => {
  ...
  it("POST /cars -> 201", async () => {
    ...
    globalExpect(body.id).toBeTypeOf("string");
    globalExpect(body.brand).toBe("Fiat");
  });
  it("GET /cars -> 200", async () => {
    ...
    const body = response.json();
    globalExpect(Array.isArray(body)).toBe(true);
    globalExpect(body).toHaveLength(1);
  });
  it("GET /cars/:id -> 200", async () => {
    ...
    globalExpect(response.statusCode).toBe(200);
    globalExpect(response.json().id).toBe(id);
  });
  it("PATCH /cars/:id -> 200", async () => {
    ...
    globalExpect(response.statusCode).toBe(200);
    const body = response.json();
    globalExpect(body.price).toBe(74990);
    globalExpect(body.mileage).toBe(12345);
  });
  it("DELETE /cars/:id -> 204", async () => {
    ...
    globalExpect(response.statusCode).toBe(204);
  });
  it("GET /cars/:id -> 404 when not found", async () => {
    const response = await app.inject({
      method: "GET",
      url: `/cars/${(0, import_node_crypto.randomUUID)()}`
    });
    globalExpect(response.statusCode).toBe(404);
    globalExpect(response.json().message).toBeTypeOf("string");
  });
