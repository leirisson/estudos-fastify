...
// src/modules/cars/cars.use.case.test.ts
...
// src/modules/cars/cars.use.case.test.ts
var InMemoryCarsRepository = class {
  items = [];
  async create(data) {
    const car = {
      ...
      ...data.mileage !== void 0 ? { mileage: data.mileage } : {},
  ...
  async update(data) {
    const index2 = this.items.findIndex((item) => item.id === data.id);
...
describe("Cars use cases", () => {
  ...
  it("creates a car", async () => {
    ...
    globalExpect(car.id).toBeTypeOf("string");
    globalExpect(car.brand).toBe("Fiat");
    globalExpect(car.version).toBeUndefined();
  });
  it("lists cars", async () => {
    ...
    globalExpect(cars).toHaveLength(1);
    globalExpect(cars[0].model).toBe("Argo");
  });
  it("gets a car by id", async () => {
    ...
    globalExpect(car.id).toBe(created.id);
  });
  it("throws when getting an unknown car", async () => {
    const sut = new GetCarByIdUseCase(carsRepository);
    await globalExpect(sut.execute({ id: (0, import_node_crypto.randomUUID)() })).rejects.toBeInstanceOf(
      CarNotFoundError
    );
  });
  it("updates a car", async () => {
    ...
    globalExpect(updated.price).toBe(74990);
    globalExpect(updated.mileage).toBe(12345);
  });
  it("throws when updating an unknown car", async () => {
    const sut = new UpdateCarUseCase(carsRepository);
    await globalExpect(
      sut.execute({ id: (0, import_node_crypto.randomUUID)(), price: 10 })
  ...
  it("deletes a car", async () => {
    ...
    globalExpect(cars).toHaveLength(0);
  });
